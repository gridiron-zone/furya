...
This key is created from `fury` and used to sign transactions. Application keys are associated with a public key prefixed by `iap` and an address prefixed by `did:fury:aa`. Both are derived from account keys generated by `fury keys add`.
...
- From [Explorer](https://fury.iobscan.io/#/staking), you can find the `Consensus Pubkey` which you declared for your validator in the Validator Detail page.
